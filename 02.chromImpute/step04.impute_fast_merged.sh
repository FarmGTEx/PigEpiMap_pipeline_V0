...
output_dir=${base}/OUTPUTDATA_merged
...
for sample in $(awk '{print $1}' ${test_marktable} | sort | uniq)
    ...
    do
    echo "java -mx6000M -jar ${impute} Apply -printonefile ${convert_dir} ${distance_dir} ${predictor_dir} ${samplemarktable} ${gsize} ${output_dir} ${sample} ${mark}"
    xsub -N step04_chr${chr}_${sample}_${mark}_onefile \
        -q root.default \
        -n 1 \
        -m 6144 \
        -o ${base}/log_merged \
        "java -mx6000M -jar ${impute} Apply -printonefile ${convert_dir} ${distance_dir} ${predictor_dir} ${samplemarktable} ${gsize} ${output_dir} ${sample} ${mark}"
        sleep 0.3
      done
     done
