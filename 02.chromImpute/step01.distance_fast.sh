...
do
echo "java -mx6000M -jar ${impute} ComputeGlobalDist -m ${i} ${convert_dir} test_samplemarktable_all.txt ${gsize} ${distance_dir}"
    xsub -N step01_${i} \
        -q root.default \
        -n 4 \
        -m 24024 \
        -o ${base}/log \
        "java -mx6000M -jar ${impute} ComputeGlobalDist -m ${i} ${convert_dir} test_samplemarktable_all.txt ${gsize} ${distance_dir}"
        sleep 0.5
done
