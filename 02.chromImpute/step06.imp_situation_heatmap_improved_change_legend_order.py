...
def make_bar_dict(input_file):
    ...
    df['value'] = df[['tissue_group_v2', 'Ontology.tissue_type', 'Ontology.stage', 'Ontology.breed', 'sex']].apply(lambda row: '\t'.join(map(str, row)), axis=1)
...
def make_big_heatmap(output_file, annotation_file):
    ...
    with open(annotation_file, 'wt') as F:
        print('tissue_group_v3', 'Ontology.tissue_type', 'Ontology.stage', 'Ontology.breed', 'sex', sep ='\t', file = F)
